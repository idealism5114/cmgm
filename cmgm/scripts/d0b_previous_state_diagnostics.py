"""Inference-only previous-micro-state utilization diagnostics for D0B.

This module deliberately does not alter the model forward path.  It loads a
trained D0B checkpoint, freezes the normal H/p trajectories from one fixed test
...
def replay_latent_trajectory(
    ...
    """
    allowed = {"normal", "zero", "sign_flip", "scaled", "shuffle", "lagged"}
    ...
    for step in range(time_steps):
        ...
        transition_input = torch.cat([long_memory[:, step], generator_prev], dim=-1)
...
def _local_generator_diagnostics(branch, H, normal_z, eps=1e-8):
    z_previous = torch.cat([torch.zeros_like(normal_z[:, :1]), normal_z[:, :-1]], dim=1)
    flat_h = H.reshape(-1, H.shape[-1])
    flat_z = z_previous.reshape(-1, z_previous.shape[-1])
    metrics = []
...
def _local_input_gradients(branch, H, normal_z, selected_steps, eps=1e-8):
    ...
    for step in selected_steps:
        ...
        h_norm = grad_h.norm(dim=-1).mean()
        z_norm = grad_z.norm(dim=-1).mean()
        values.append({
            "time": step,
            "grad_h": h_norm.item(),
...
def run_previous_state_diagnostics(model, x_batch: torch.Tensor,
    ...
    if model.variant != "switching_latent_balanced_readout":
        raise ValueError("diagnostics require the exact D0B variant")
    branch = model.switching_latent_transformer
    if not branch.balanced_readout or branch.use_latent_memory or branch.use_dynamic_slope:
        raise ValueError("diagnostics require plain D0B, not a later D-series variant")
    model.eval()
...
def _print_report(result, H, p, batch_shape):
    ...
    print("\n[D0B previous-state diagnostics: inference only]")
    ...
    print(
        "  [D0B zero-Zprev sanity] max H diff="
    ...
    for state, (mean, maximum) in enumerate(result["zero_previous_candidate_diffs"]):
        print(
            f"  [D0B zero-previous-state] candidate state{state} "
            f"mean/max diff={mean:.6e}/{maximum:.6e}"
        )
    print(
        "  [D0B zero-previous-state] Z trajectory mean diff="
        f"{result['zero_previous_trajectory_mean_diff']:.6e}"
    )
    for step, value in result["zero_previous_time_diffs"].items():
        print(f"  [D0B zero-previous-state t={step}] mean |delta Z_t|={value:.6e}")

    print("\n  Intervention                         Z_T mean/max       h_micro mean/max   h_temporal mean/max prediction mean/max")
    for key in ("normal", "zero_previous", "sign_flip", "scale_0.5", "scale_1.5", "shuffle", "lagged", "zero_micro"):
    ...
    print(
        "  [D0B shuffled-previous-state] unchanged batch positions="
        f"{result['shuffle_unchanged_fraction']:.2%} permutation="
        f"{result['permutation'].tolist()}"
    )

    print("\n  [D0B local previous-state sensitivity]")
    ...
    print("  [D0B generator-local candidate-energy gradients]")
    ...
    for state, values in result["forced"].items():
        print(
            f"  [D0B recurrence interaction state {state}] "
...
def main():
    ...
    print(
        f"[Checkpoint] D0B best={args.checkpoint.resolve()} "
