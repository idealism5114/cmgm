"""One pre-GNN local skip experiment; original D0B protocol and controlled inference."""
import argparse
...
from cmgm.scripts.d0b_pregnn_diagnostics import (
    VARIANT,BASE,DISPLAY,initialization_check,gradient_diagnostics,structural_sanity,commodity_order_check,assert_backbone,
)
from cmgm.scripts.d0b_pregnn_analysis import collect_with_controls,primary_arrays,residual_analysis,ablation_analysis,amplitude_stats
...
def validate_protocol(args):
    try:
        ...
        raise ValueError(str(error).replace('Hybrid', 'PreGNNLocalSkip')) from error
...
def export(r,out):
    for filename,key in [('training_history','history'),('residual_diagnostics','residual'),
...
def run_pregnn(args,device,data):
    from cmgm.scripts.d0b_pregnn_report import build_comparison,write_report
    validate_protocol(args)
    if not args.no_cuda and device.type!='cuda':raise RuntimeError('PreGNNLocalSkip requires GPU unless CPU is explicitly requested; no silent fallback')
    ...
    out=Path(args.pregnn_report_dir)/datetime.now().strftime('%Y%m%d_%H%M%S');out.mkdir(parents=True,exist_ok=False)
    set_seed(args.seed)
    ...
    r=dict(display=DISPLAY,variant=VARIANT,seq_len=20,seed=42,status='PREFLIGHT',training_executed=False,primary_case=None,
        ...
        protocol='Only one shared zero-initialized pre-GNN local residual added; original raw Q/K, all 8 priors and complete D0B global path unchanged. Formal selection uses original multi-horizon validation objective; pooled VAL5 is secondary only.')
    files=['cmgm/config.py','cmgm/models/hetero_mixhop_model.py','cmgm/graph/adaptive_graph.py','cmgm/training/train.py',
        ...
        'cmgm/scripts/d0b_pregnn_local_skip.py','cmgm/scripts/d0b_pregnn_diagnostics.py','cmgm/scripts/d0b_pregnn_report.py','cmgm/scripts/d0b_pregnn_analysis.py',
    ...
    def capture(active,stage):
        ...
        r['gradients'][stage]=g
        with diagnostic_context(active),torch.no_grad():
            active(test_x)
            r['residual'][stage]=dict(final_weight_norm=g['final_weight_norm'],first_weight_norm=g['first_weight_norm'],
                first_gradient_norm=g['norms']['residual_first'],final_gradient_norm=g['norms']['residual_final'],
                fixed_TEST_amplitude=amplitude_stats(active.last_residual.cpu().numpy()))
        return g
    with diagnostic_context(model):
        ...
        assert r['sanity']['shared_init']['D0B_params']==520549 and r['sanity']['shared_init']['PreGNN_params']==524805
        print(f'[{DISPLAY} shared init] {r["sanity"]["shared_init"]}',flush=True)
        r['sanity']['initial']=structural_sanity(model,test_x)
        r['sanity']['commodity_order']=commodity_order_check(data,model)
        ...
        del baseline
    r['sanity']['PASS']=r['sanity']['shared_init']['PASS'] and r['sanity']['initial']['PASS'] and r['sanity']['legacy_D0B_reference'] and r['sanity']['commodity_order']['PASS'] and r['sanity']['initial_gradient']['PASS']
    ...
    history=train(model,data['loaders']['train'],data['loaders']['val'],torch.empty((2,0),dtype=torch.long,device=device),
        ...
        checkpoint_metadata=dict(variant=VARIANT,display_name=DISPLAY,seed=args.seed,seq_len=20,git_sha=r['git_sha'],
            parameter_count=524805,added_parameters=4256,residual='shared 128-32-4; final bias=False and zero initialized',
    ...
    r['sanity']['PASS']=r['sanity']['PASS'] and r['sanity']['best']['PASS'] and r['sanity']['commodity_order']['PASS'] and r['sanity']['initial_gradient']['PASS']
    if not r['sanity']['PASS']:
        r['status']='STOPPED: best sanity failed; DO NOT INTERPRET PERFORMANCE'
        export(r,out);write_report(r,out);raise AssertionError(r['status'])
    controlled,permutation=collect_with_controls(model,data['loaders'],device)
    arrays=primary_arrays(controlled)
    r['shuffle_permutation']=permutation
    r['residual']['full_loader']=residual_analysis(controlled,data['feature_names'][cs:ce])
    r['ablations']=ablation_analysis(base_arrays,controlled)
    ...
    m=r['metrics']['PreGNNLocalSkip']['test'];zero=population_metrics(np.zeros_like(arrays['test']['target']),arrays['test']['target'])
    legacy={**m,'Hit_Ratio':m['Hit']}
    return dict(variant=DISPLAY,params=524805,time=r['train_time_seconds'],MAE=m['MAE'],MSE=m['MSE'],RMSE=m['RMSE'],
...
def main():
    ...
    parser.add_argument('--pregnn-report-dir',type=Path,default=Path('experiments')/'d0b_pregnn_local_skip')
    ...
    run_pregnn(args,device,build_data(args))
